...
class TwitterAnalyzer:
    ...
    def tweets_on_map_by_trend(self):
        """
        Creates a map with markers tweet geolocation by specific trend.
        """
        trends_with_geodata = ap.twitter_trends_of_geodata_tweets(
            self.collection)
        TwitterAnalyzer.static_geodata_trend_options = list(
            map(lambda trend_dict: trend_dict.get('trend'), trends_with_geodata))
        trend = st.selectbox(
            label='Trend', options=TwitterAnalyzer.static_geodata_trend_options)

        if st.button('Show'):
            tweets = list(ap.twitter_tweets_with_geodata_by_trend(
                self.collection, trend))
            for tweet in tweets:
